...
//#ifdef Q_WS_WIN
#include "cl_DataForVxWorks_Su30SM.h"


using namespace SU30SM;
cl_DataForVxWorks_SU30SM::cl_DataForVxWorks_SU30SM(void):GenericData()
{
...
void cl_DataForVxWorks_SU30SM::newTransmitObject()
{
    if(transmit!=0)
    {
...
void cl_DataForVxWorks_SU30SM::refreshFormTargets()
{
    QStringList value;

    //qDebug("refershTarget=%d\n",middleBufferData.NumberTarget);
    ...
    for(int i=0;i<numTargets;i++)
    {
        ...
        value<<QString::number(tar->Beg_num_target,FORM,PRE);   //21
...
void cl_DataForVxWorks_SU30SM::refreshRegime()
{
...
int cl_DataForVxWorks_SU30SM::targetNumbers()
{
    return numTargets;
}
int cl_DataForVxWorks_SU30SM::targetAirNum()
{
    return middleBufferData.airtarget_Q;
}
void cl_DataForVxWorks_SU30SM::refreshData()
{
        qDebug("cl_DataForVxWorks_SU30SM::refreshData()\n");
        bufferData=middleBufferData;
        qDebug("end of cl_DataForVxWorks_SU30SM::refreshData()\n");
        ...
        qDebug("cl_DataForVxWorks_SU30SM::refreshData()-refreshFormTargets\n");
        refreshFormTargets();
        qDebug("end of cl_DataForVxWorks_SU30SM::refreshData()-refreshFormTargets\n");
        refreshFormAtmGeo();
...
void cl_DataForVxWorks_SU30SM::refreshFormAtmGeo(void)
{
        QStringList value;
        ItemTable* item=0;
...
void cl_DataForVxWorks_SU30SM::refreshFormKinAircraft(void)
{
        QStringList value;
        ItemTable* item=0;
...
void cl_DataForVxWorks_SU30SM::refreshFormConst(void)
{
    QStringList value;
    ItemTable* item=0;

    Tkinematiks_aircraftConst *pKinConst=&middleBufferData.Kinematiks_aircraftConst;
    Tkinematiks_aircraft *pKin=&middleBufferData.DesArjObject.Kinematiks_aircraft;
...
void cl_DataForVxWorks_SU30SM::refreshFormCommonConst(void)
{
    QStringList value;
    ItemTable* item=0;
...
void cl_DataForVxWorks_SU30SM::createFormAtmGeo(void)
{
...
void cl_DataForVxWorks_SU30SM::createFormTargets(void)
{
        QVector <ItemTable*> items;
...
void cl_DataForVxWorks_SU30SM::createFormConst(void)
{
        QVector <ItemTable*> items;
...
void cl_DataForVxWorks_SU30SM::createFormKinAircraft(void)
{
        QVector <ItemTable*> items;
...
void cl_DataForVxWorks_SU30SM::createFormCommonConst(void)
{
    QVector <ItemTable*> items;
...
void cl_DataForVxWorks_SU30SM::sendData()
{
    qDebug("call cl_DataForVxWorks_Su35_SM::sendData()\n");
    ...
    //dstr->N_BegFromPult=        list[2].toInt();    //3
    //dstr->N_EndFromPult=        list[3].toInt();    //4
    //dstr->TotalNumberVariants=  list[4].toInt();    //5
    //dstr->NumberTarget=         list[5].toInt();    //6
    //dstr->airtarget_Q=          list[6].toInt();    //7
    ...
    bufferData.NumberRealization=list[20].toFloat(); //21
...
void cl_DataForVxWorks_SU30SM::setData(char* buf)
{
    middleBufferData=*((TDSTRUCTVISULISATION*)buf);
    //qDebug("NumberTarget=%d\n",middleBufferData.NumberTarget);
...
void cl_DataForVxWorks_SU30SM::callSlotResived(char* data)
{
    //если линух необходимо довернуть данные
    setData(data);
    emit GenericData::ReciveData();
}
unsigned int cl_DataForVxWorks_SU30SM::counter()
{
    return middleBufferData.Counter;
}
void cl_DataForVxWorks_SU30SM::setControlRegime(ControlReg reg)
{
    switch(reg)
    {
    ...
    default:                {qDebug("cl_DataForVxWorks_Su35_SM: not correct reg\n");}
    };
}

void cl_DataForVxWorks_SU30SM::setAvtoTiaga(bool flag)
{
    if(flag==true) bufferData.ComRegimeStend.AvtTiaga=1;
    else bufferData.ComRegimeStend.AvtTiaga=0;
}
bool cl_DataForVxWorks_SU30SM::avtoTiaga()
{
    if(middleBufferData.ComRegimeStend.AvtTiaga==1) return true;
    else return false;
}
void cl_DataForVxWorks_SU30SM::setSauIdelPilot(int flag)
{
...
int cl_DataForVxWorks_SU30SM::sauIdelPilot()
{
...
void cl_DataForVxWorks_SU30SM::setDetAerod1(int flag)
{
    if(flag==0)
...
int cl_DataForVxWorks_SU30SM::detAerod1()
{
...
void cl_DataForVxWorks_SU30SM::setSerpent(bool flag)
{
    if(flag==true)
...
bool cl_DataForVxWorks_SU30SM::serpent()
{
    if(middleBufferData.ComRegimeStend.Serpent==1)
        return true;

    return false;
}

void cl_DataForVxWorks_SU30SM::setManevrTarget(bool flag)
{
    if(flag==true) bufferData.ComRegimeStend.ManevrTaget=1;
    else bufferData.ComRegimeStend.ManevrTaget=0;
}
bool cl_DataForVxWorks_SU30SM::manevrTarget()
{
    if(middleBufferData.ComRegimeStend.ManevrTaget==1)
        return true;

    return false;
}
void cl_DataForVxWorks_SU30SM::setHrv0(double value)
{
    bufferData.Kinematiks_aircraftConst.Hrvrf_C=value;
}
void cl_DataForVxWorks_SU30SM::setHrvmin(double value)
{
    bufferData.Kinematiks_aircraftConst.Hrvrfmin_C=value;
}
void cl_DataForVxWorks_SU30SM::setHrvmax(double value)
{
    bufferData.Kinematiks_aircraftConst.Hrvrfmax_C=value;
}
void cl_DataForVxWorks_SU30SM::setKursRv(double value)
{
    bufferData.Kinematiks_aircraftConst.psirvrf_C=value;
}
void cl_DataForVxWorks_SU30SM::alfaSM(double value)
{
    bufferData.CommonConst.AlfaSM=value;
}
void cl_DataForVxWorks_SU30SM::Vminair(double value)
{
    bufferData.CommonConst.Vmin_C=value;
}
void cl_DataForVxWorks_SU30SM::Hmaxair(double value)
{
