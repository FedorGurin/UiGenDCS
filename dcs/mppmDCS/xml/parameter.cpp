...
Parameter::Parameter(const QDomElement& element,GenericNode *parent):GenericNode()
{
    ...
    modeEdit=Parameter::DEC;
    modeIndication=Parameter::DEFAULT_IND;
...
Parameter::Parameter(const Parameter* param, Node *glParent /*глобальный предок*/):GenericNode()
{
    ...
    isHaveHelps=    param->isHaveHelps;
    fileNameHelps=  param->fileNameHelps;
    ...
    cycle_request=  param->cycle_request;
    bytes=          param->bytes;
...
void Parameter::addMassiveParam(void)
{
    if(howElements>1)
    {
        ...
        for(int i=1;i<howElements;i++)
        {
            ...
            //((GenericNode*)(this->parent))->addChild(newParam);
